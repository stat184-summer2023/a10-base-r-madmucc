...
```{r}
Counting <- data.frame(matrix(ncol=4, nrow=3))
for(i in 1:3){
  for(j in 1:4){
    for(k in 1:1){
      Counting[i, j] <- (i + j)-1
...
plotNormal <- function(repetitions, mean, sd){
  numbers <- rnorm(repetitions, mean = mean, sd = sd)
  hist(numbers, main = "Random Normal Numbers", xlab="Values")
}
#5A
plotNormal(repetitions=100,mean=2,sd=0.5)
#5B
plotNormal(repetitions=1000,mean=0,sd=1)
```
...
plotNormal <- function(repetitions, mean, sd){
  numbers <- rnorm(repetitions, mean = mean, sd = sd)
  hist(numbers, main = sprintf("Histogram from a Normal (%s, %s) Distribution", mean, sd), xlab="Values")
}
plotNormal(repetitions=1000,mean=0,sd=1)
```
